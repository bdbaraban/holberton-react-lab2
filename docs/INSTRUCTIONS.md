In this lab, we will refactor our Todo List app from Lab 1. We have provided a reference implementation for you to use as a starting point should you wish. To use the reference implementation:

```bash
$ git clone git@github.com:p0x6/holberton-react-lab2.git
```
...
Now when we wire up our app with the redux store we’ll get some visual feedback of our success! Ok, we need to make the store available to our component tree. To do this we’ll use React’s `context` API through the `react-redux` bindings library. Just after your `store` code, create an `App` component that wraps `TodoApp` with the `Provider` we importer earlier. Then pass your Redux store as a prop to the `Provider` which will make it accessible to any component in our app hierarchy.
...
In `src/todo-app.jsx` let’s remove the old component level state. This is easy to do because we can just delete the entire constructor from the class. Don’t worry that we are also getting rid of the method bindings. We won’t be using those any more as we refactor to Redux. Make sure to also remove the `todos` prop being passed to `TodoList`.
...
## Milestone 3: Access State from Components
...
The `connect` function has an interesting function signature. The first argument should be a function that will recieve the current state as an argument. It should return an object with key value pairs that will be passed to the connected component as props. The function is commonly called `mapStateToProps` since this name describes exactly what it does, but the name is not important. This function is where you can select the branches of the state tree your component is interested in and it will directly recieve them as props – no matter how deeply nested it is. No more passing props down the component tree!
...
Take a moment to study how we use `connect` – it accepts a state-mapping function, returns a new function that accepts a component, and that second function returns a new connected component that wraps ours and is responsible for passing it the props we specified in the mapper. The connected wrapper component is subscribed to updates from the store and will rerender when state changes, passing the new values as props to our wrapped component.
...
Next we’ll write an action creator. This is commonly done in a separate file, but we’re doing it here to show you the pattern. An action creator is simply a function that creates an action, typically by using some input it receives. After the component code add the `addTodo` action creator

```jsx
const addTodo = text => ({
  type: 'ADD_TODO',
  payload: { text, id: new Date().getTime() }
});
```
...
This function is analagous to `mapStateToProps`: it will recieve the store’s dispatch method as an argument, and it is expected to return an object of key value pairs that will be passed to the component as props. Here our values are functions that invoke the `dispatch` method with an action.
...
handleSubmit() {
  ...
  this.props.handleAdd(this.state.todoText.trim());
  this.setState({
    todoText: ''
  });
}
...
If, in a frenzy of uncontrolled excitement, you fired up your app and tried adding a todo, you may have been disappointed to see nothing happen. Although we are currently dispatching an action, there is a second piece required to update the state. A reducer must be explicitly written to handle the action you are dispatching, so our work is not yet done.

In `src/reducer.js` we’ll now make our `todos` reducer actually do something interesting instead of just returning the current state. The action type we used in the action creator was `ADD_TODO` so let’s add a `switch` `case` to handle that type. Here is the updated reducer for reference:
